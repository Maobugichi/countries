  ...
  const {theme,setTheme} = useContext(ThemeContext)
    return(
      <>
       {data.map((item,index) => {
        return(
          <Link className={`transition-all duration-300 w-[80%] mx-auto lg:w-[23%]  h-[400px] ${theme.sbg}`} to={`moreinfos/${index}`} state={{data:data,name:item.name,theme:theme}}>
            ...
            <div  className="h-[40%]  grid items-center">
                <div className=" h-[70%] w-[80%] mx-auto flex flex-col justify-between ">
                <h2 className={`font-bold ${theme.txt}`}>{item.name}</h2>
                <ul className={`text-${theme.lightText}`}>
